...
const { SESSION_DB_INSTANCE } = require(path.join(__dirname, "..","..","DB","dbInstances"));
const { SESSION_DB_NAME, SESSION_TABLE_NAME, CHECK_LOGIN_DETAILS,SESSION_ID, USER_EMAIL, EXPIRES_AT, ROLE, CREATED_AT, SESSION_DB_CREATION_SQL, CREATE_SESSION_DETAILS, SESSION_LOGOUT_DETAILS } = require(path.join(__dirname, "..","..","DB", "sessionConstants"));
...
class SessionHelper{
    ...
    static async logout(sessionInformation){
        try{
            let sqlString = `SELECT * FROM ${SESSION_TABLE_NAME} where `;
            ...
            SESSION_LOGOUT_DETAILS.map(key => {
                let value = sessionInformation[key];
                if(value){
                    givenInformation.push(key);
                    givenValues.push(value);
