...
const { USER_REGISTER_REQUEST_RULE, RESTAURANT_OWNER_REGESTER_REQUEST_RULE } = require(path.join(__dirname, "registerConstants"));
...
const checkRestaurantOwnerRegisterBody = (req, res, next) => {
    const reqCheck = checkRequestFormate(RESTAURANT_OWNER_REGESTER_REQUEST_RULE, req);
    if(reqCheck.valid){
        next();
    }
    else{
