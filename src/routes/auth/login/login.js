...
const { checkUserLoginBody, checkRestaurantOwnerLoginBody } = require(path.join(__dirname, "utilities"));
...
loginRouter.post(USER_LOGIN_ROUTE, checkUserLoginBody,async(req, res) => {
    try{
...
loginRouter.post(RESTAURANT_OWNER_LOGIN_ROUTE, checkRestaurantOwnerLoginBody,async(req, res) => {
    try{
        const user = await RestaurantOwnerBuilder.login(req.body);
        if(user.error){
            let statusCode = user.status;
            return res.status(statusCode).json({
                error: user.error,
        ...
        let statusCode = user.status;
            return res.status(statusCode).json({
                error: user.error,
                user: user.restaurant_owner.getJson(),
                [SESSION_ID]: user[SESSION_ID],
                [CREATED_AT]: user[CREATED_AT],
                [EXPIRES_AT]: user[EXPIRES_AT]
            });
